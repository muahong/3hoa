...
(function () {
  ...
  /* ---------------- Bản nhạc ---------------- */
  const TRACKS = {
    // Khi chơi trong mê cung: vui nhộn, 128 BPM, 8 ô nhịp
    game: {
      bpm: 128,
      ...
      lead: expand([
        ...
        ['G5', 2], ['F5', 2], ['E5', 2], ['D5', 2], ['E5', 2], ['D5', 2], ['C5', 4],
        ['E5', 2], ['G5', 2], ['C6', 2], ['G5', 2], ['E5', 2], ['G5', 2], ['C6', 4],
        ['D6', 2], ['C6', 2], ['B5', 2], ['C6', 2], ['D6', 2], ['C6', 2], ['A5', 4],
        ['F5', 2], ['G5', 2], ['A5', 2], ['B5', 2], ['C6', 4], ['B5', 2], ['A5', 2],
        ['G5', 2], ['E5', 2], ['D5', 2], ['E5', 2], ['C5', 8]
      ]),
      bass: bassLine(['C3', 'A2', 'F2', 'G2', 'C3', 'A2', 'F2', 'G2'], true),
      bassVol: 0.12,
      kick: 'x...x...x...x...',
      snare: '....x.......x...',
      hat: 'x.x.x.x.x.x.x.x.'
    },
    // Menu, bài học, hỏi đáp: êm dịu như đêm trăng, 84 BPM
    menu: {
      bpm: 84,
  ...
  const Sfx = {
    ...
    play(name) {
      ...
      try {
        switch (name) {
          case 'dot':                                   // ăn hạt sáng (waka luân phiên cao thấp)
            this._waka = 1 - this._waka;
            this._tone(this._waka ? 520 : 700, { type: 'square', dur: 0.05, vol: 0.09, slide: this._waka ? 760 : 480 });
            break;
          case 'power':                                 // ăn ngôi sao
            [523.25, 659.25, 783.99, 1046.5, 1318.5].forEach((f, i) => this._tone(f, { t: i * 0.05, dur: 0.12, vol: 0.22, type: 'square' }));
            this._tone(2093, { t: 0.25, dur: 0.4, vol: 0.12, type: 'sine' });
            break;
          case 'clock':                                 // tìm đúng đồng hồ
            ...
            this._tone(1046.5, { t: 0.3, dur: 0.3, vol: 0.28, type: 'sine' });
            this._tone(1567.98, { t: 0.42, dur: 0.35, vol: 0.14, type: 'sine' });
            break;
          case 'correct':
            this._tone(523.25, { t: 0, dur: 0.1, vol: 0.32 });
            this._tone(659.25, { t: 0.09, dur: 0.1, vol: 0.32 });
            ...
            break;
          case 'wrong':                                 // chọn nhầm đồng hồ / trả lời sai
            this._tone(220, { type: 'sawtooth', dur: 0.28, vol: 0.22, slide: 140 });
            this._tone(110, { type: 'square', dur: 0.28, vol: 0.12, slide: 70 });
            break;
          case 'eatghost':                              // bắt được ma đang buồn ngủ
            this._tone(300, { type: 'square', dur: 0.18, vol: 0.2, slide: 1400 });
            this._tone(1200, { type: 'sine', t: 0.15, dur: 0.2, vol: 0.18, slide: 2000 });
            this._noiseHit({ t: 0.05, dur: 0.15, vol: 0.12, f0: 2500, q: 0.8 });
            break;
          case 'hurt':                                  // bị ma bắt
            this._noiseHit({ dur: 0.5, vol: 0.35, filter: 'lowpass', f0: 2000, f1: 90, q: 0.7 });
            this._tone(400, { type: 'sawtooth', dur: 0.5, vol: 0.25, slide: 60 });
            [392, 349.23, 311.13].forEach((f, i) => this._tone(f, { t: 0.25 + i * 0.18, dur: 0.2, vol: 0.2, type: 'triangle' }));
            break;
          case 'ghostwake':                             // ma tỉnh dậy (hết sức mạnh)
            this._tone(600, { type: 'square', dur: 0.08, vol: 0.12, slide: 900 });
            break;
          case 'levelclear':                            // qua màn
            [392, 523.25, 659.25, 783.99, 1046.5].forEach((f, i) => this._tone(f, { t: i * 0.09, dur: i === 4 ? 0.5 : 0.14, vol: 0.3, type: 'square' }));
            break;
          case 'unlock':                                // mở khóa màn mới
            [659.25, 783.99, 987.77, 1318.5, 1567.98, 2093].forEach((f, i) => this._tone(f, { t: i * 0.07, dur: 0.16, vol: 0.22, type: 'sine' }));
            break;
          case 'record':
          ...
          case 'go':
            ...
            break;
          case 'target':                                // mục tiêu mới xuất hiện
          ...
          case 'hint':
            ...
            break;
          case 'bonus':                                 // ăn hết hạt sáng
            [783.99, 987.77, 1174.66, 1567.98].forEach((f, i) => this._tone(f, { t: i * 0.06, dur: 0.11, vol: 0.26 }));
            break;
          case 'click':
            this._tone(700, { type: 'sine', dur: 0.05, vol: 0.15, slide: 900 });
            break;
          case 'turn':                                  // đổi hướng (rất nhẹ)
            this._tone(900, { type: 'sine', dur: 0.03, vol: 0.05 });
            break;
        }
