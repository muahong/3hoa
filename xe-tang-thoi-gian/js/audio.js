/* ============================================================
   audio.js – Âm thanh cho Xe Tăng Thời Gian
   - Sfx: tiếng pháo, nổ, đạn nảy, chuông đúng/sai... tổng hợp bằng Web Audio (không cần file mp3)
   - Music: nhạc nền chiptune kiểu hành khúc, lập lịch chính xác theo AudioContext
   - Voice: giọng đọc tiếng Việt (Web Speech API) đọc câu hỏi, đáp án, lời khen
...
(function () {
  ...
  /* ---------------- Bản nhạc ---------------- */
  const TRACKS = {
    // Khi chơi: hành khúc vui nhộn, 124 BPM, 8 ô nhịp
    game: {
      bpm: 124,
      leadType: 'square', leadVol: 0.08, leadCut: 2600,
      lead: expand([
        ['C5', 2], ['E5', 2], ['G5', 2], ['E5', 2], ['C5', 2], ['G4', 2], ['C5', 4],
        ['D5', 2], ['F5', 2], ['A5', 2], ['F5', 2], ['D5', 2], ['A4', 2], ['D5', 4],
        ['E5', 2], ['G5', 2], ['C6', 2], ['G5', 2], ['E5', 2], ['C5', 2], ['E5', 4],
        ['F5', 2], ['E5', 2], ['D5', 2], ['C5', 2], ['D5', 6], [null, 2],
        ['G5', 2], ['G5', 2], ['A5', 2], ['B5', 2], ['C6', 4], ['G5', 4],
        ['A5', 2], ['G5', 2], ['F5', 2], ['E5', 2], ['F5', 4], ['D5', 4],
        ['E5', 2], ['C5', 2], ['E5', 2], ['G5', 2], ['A5', 2], ['G5', 2], ['E5', 4],
        ['D5', 2], ['E5', 2], ['F5', 2], ['D5', 2], ['C5', 8]
      ]),
      bass: bassLine(['C3', 'F2', 'C3', 'G2', 'C3', 'F2', 'C3', 'G2'], true),
      bassVol: 0.13,
      kick: 'x...x...x...x...',
      snare: '....x.......x...',
      hat: 'x.x.x.x.x.x.x.x.'
    },
    // Menu: nhẹ nhàng như tiếng tích tắc đồng hồ, 84 BPM, 4 ô nhịp
    menu: {
      bpm: 84,
      leadType: 'triangle', leadVol: 0.11, leadCut: 3000,
      lead: expand([
        ['C4', 2], ['E4', 2], ['G4', 2], ['C5', 2], ['G4', 2], ['E4', 2], ['C4', 2], ['G4', 2],
        ['A3', 2], ['C4', 2], ['E4', 2], ['A4', 2], ['E4', 2], ['C4', 2], ['A3', 2], ['E4', 2],
        ['F3', 2], ['A3', 2], ['C4', 2], ['F4', 2], ['C4', 2], ['A3', 2], ['F3', 2], ['C4', 2],
        ['G3', 2], ['B3', 2], ['D4', 2], ['G4', 2], ['D4', 2], ['B3', 2], ['G3', 2], ['D4', 2]
      ]),
      bass: bassLine(['C3', 'A2', 'F2', 'G2'], false),
      bassVol: 0.09,
      kick: '', snare: '', hat: 'x...x...x...x...'
    }
  };
  ...
  const Sfx = {
    ...
    play(name) {
      ...
      try {
        switch (name) {
          case 'shot':                                  // pháo xe tăng bắn
            this._noiseHit({ dur: 0.18, vol: 0.45, filter: 'lowpass', f0: 1800, f1: 200, q: 0.8 });
            this._tone(140, { type: 'sine', dur: 0.22, vol: 0.5, slide: 45 });
            this._tone(900, { type: 'square', dur: 0.06, vol: 0.08, slide: 300 });
            break;
          case 'explode':                               // robot nổ
            this._noiseHit({ dur: 0.5, vol: 0.45, filter: 'lowpass', f0: 2400, f1: 100, q: 0.8 });
            this._tone(110, { type: 'sine', dur: 0.4, vol: 0.4, slide: 35 });
            this._tone(1500, { type: 'triangle', t: 0.05, dur: 0.15, vol: 0.12, slide: 500 });
            break;
          case 'ricochet':                              // đạn nảy khỏi bảng sai
            this._tone(2400, { type: 'triangle', dur: 0.25, vol: 0.22, slide: 900 });
            this._noiseHit({ dur: 0.12, vol: 0.2, f0: 4000, f1: 1500, q: 2 });
            break;
          case 'spawn':                                 // robot bay tới
            this._noiseHit({ dur: 0.35, vol: 0.06, f0: 300, f1: 1200, q: 0.6 });
            this._tone(600, { type: 'sine', dur: 0.2, vol: 0.06, slide: 900 });
            break;
          case 'target':                                // chọn mục tiêu
            this._tone(700, { type: 'sine', dur: 0.06, vol: 0.14, slide: 1000 });
            this._tone(1000, { type: 'sine', t: 0.06, dur: 0.08, vol: 0.12 });
            break;
          case 'move':                                  // xe tăng chạy
            this._noiseHit({ dur: 0.08, vol: 0.05, filter: 'lowpass', f0: 500, f1: 200, q: 0.5 });
            break;
          case 'question':
            this._tone(1046.5, { type: 'sine', dur: 0.12, vol: 0.14 });
            this._tone(1318.5, { type: 'sine', t: 0.11, dur: 0.2, vol: 0.14 });
            break;
          case 'correct':
          ...
          case 'wrong':
            ...
            break;
          case 'breach':                                // robot tới xe tăng
            this._noiseHit({ dur: 0.7, vol: 0.5, filter: 'lowpass', f0: 2500, f1: 80, q: 0.7 });
            this._tone(90, { type: 'sine', dur: 0.55, vol: 0.5, slide: 28 });
            this._tone(660, { type: 'square', t: 0.25, dur: 0.14, vol: 0.12 });
            this._tone(660, { type: 'square', t: 0.45, dur: 0.14, vol: 0.12 });
            break;
          case 'heart':                                 // hồi máu
            this._tone(880, { type: 'sine', dur: 0.12, vol: 0.26 });
            this._tone(1174.7, { type: 'sine', t: 0.1, dur: 0.14, vol: 0.26 });
            this._tone(1760, { type: 'sine', t: 0.2, dur: 0.3, vol: 0.2 });
            break;
          case 'hint':
          ...
          case 'tick':
            ...
            break;
          case 'tock':                                  // tích tắc đồng hồ
            this._tone(1200, { type: 'sine', dur: 0.04, vol: 0.12, slide: 700 });
            break;
          case 'go':
          ...
          case 'warn':
            ...
            break;
          case 'win':                                   // hoàn thành màn
            [523.25, 659.25, 783.99, 1046.5, 783.99, 1046.5, 1318.5].forEach((f, i) => this._tone(f, { t: i * 0.1, dur: i === 6 ? 0.6 : 0.15, vol: 0.3, type: i % 2 ? 'square' : 'triangle' }));
            break;
          case 'lose':
            [523.25, 466.16, 415.3, 349.23].forEach((f, i) => this._tone(f, { t: i * 0.16, dur: 0.22, vol: 0.28, type: 'triangle' }));
            break;
          case 'unlock':                                // mở khóa màn mới
            [392, 523.25, 659.25, 783.99, 1046.5, 1318.5].forEach((f, i) => this._tone(f, { t: i * 0.08, dur: i === 5 ? 0.7 : 0.14, vol: 0.3 }));
            this._tone(2093, { t: 0.5, dur: 0.7, vol: 0.14, type: 'sine' });
            break;
          case 'record':
